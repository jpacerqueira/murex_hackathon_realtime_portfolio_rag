...
def build_root_agent() -> Agent:
    ...
    instruction = build_instruction(read_only=read_only, mutating=mutating)

    return Agent(
        model=settings.adk_model,
        name="trade_blotter_concierge",
        description=(
            "ADK agent that drives a Trade Blotter MCP server through an HTTP "
            "bridge. Read-only tools execute directly; mutating tools require "
            "human approval via a LongRunningFunctionTool."
        ),
        instruction=instruction,
        tools=[approval_tool, *tools],
